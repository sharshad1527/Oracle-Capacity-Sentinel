...
SCRIPT_NAME="Scripts/termux_engine.py"
...
show_menu() {
    clear
    echo -e "${YELLOW}==========================================${NC}"
    echo -e "   Oracle Cloud Control Panel (TERMUX)    "
    echo -e "${YELLOW}==========================================${NC}"
    ...
    case $choice in
        1)
            if is_running; then
                ...
                tmux new-session -d -s $SESSION_NAME "python $SCRIPT_NAME; read -p 'Press Enter to exit...'"
                echo "Done!"
                sleep 2
            fi
            ;;
